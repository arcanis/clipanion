...
import type {CliOptions, BaseContext}   from '../../sources/advanced';
import {Cli, Command, Option, Builtins} from '../../sources/advanced';
...
describe(`Advanced`, () => {
  ...
  it(`should expose Cli options on the MiniCli`, async () => {
    ...
    expect(JSON.parse(output)).toEqual(binaryInfo);
  });
  ...
  it(`derives positional argument names from the property name`, async () => {
    ...
    expect(cli.usage(CommandA)).toEqual(`$ ... workspace <workspaceName> [extra] <scriptName>\n`);
  });
  ...
  it(`derives rest argument names from the property name`, async () => {
    ...
    expect(cli.usage(CommandA)).toEqual(`$ ... clean <workspaceNames> <workspaceNames> ...\n`);
  });
  ...
  it(`should print flags with a description separately`, async () => {
    ...
    expect(cli.usage(CommandA, {detailed: true})).toMatch(/\$ \.\.\. greet \[--message #0\]\n\nOptions\n\n\s*--verbose\s+Log output\n\s*--output #0\s+The output directory\n/);
  });
